  ...
  async init() {
    ...
    this._initPromise = (async () => {
      try {
        ...
        if ((!this.state.institutions || this.state.institutions.length === 0) && this.state.googleUserId) {
          debug('[StateManager] Checking backend for all user items to migrate...');

          try {
            // Fetch ALL items from backend for this user
            const BACKEND_URL = 'https://api.sheetlink.io';
            const response = await fetch(`${BACKEND_URL}/plaid/items?user_id=${encodeURIComponent(this.state.googleUserId)}`);

            if (response.ok) {
              const backendItems = await response.json();
              debug(`[StateManager] Found ${backendItems.length} items on backend`);

              if (backendItems.length > 0) {
                // Fetch details for each item and add to institutions array
                const institutions = [];
                for (const item of backendItems) {
                  try {
                    ...
                    if (itemInfoResponse.ok) {
                      ...
                      debug(`[StateManager] Migrated: ${itemInfo.institution_name}`);
                    }
                  } catch (error) {
                    console.error(`[StateManager] Failed to fetch details for item ${item.item_id}:`, error);
                  }
                }
                ...
                // Set first institution as legacy itemId for backward compatibility
                ...
                debug(`[StateManager] Migration complete. Migrated ${institutions.length} institution(s) from backend`);
              }
            }
          } catch (error) {
            console.error('[StateManager] Migration from backend failed:', error);

            // Fallback: Try to migrate from local storage if available
            if (this.state.itemId) {
              debug('[StateManager] Falling back to local storage migration...');
              ...
              await chrome.storage.sync.set({ institutions: this.state.institutions });
              debug('[StateManager] Fallback migration complete:', this.state.institutionName);
            }
          }
        }
